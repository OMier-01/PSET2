...
range(d$Game1Angry1, na.rm = TRUE)   #na.rm is to take out NA from calculations 
hist(d$Game1Angry1)
```
...
```{r echo=TRUE}
# To see which participants are excluded from the study
# excluded_d = d %>% 
#  filter(!is.na(DoNotUse))  # to see which rows do not have NA
# view(excluded_d)

# your code here: exclude rows 2, 90, and 91 that are marked as "DoNotUse"
filtered_d = d %>% 
...
tiny_demo_d %>% pivot_longer(cols=-c("Subject", "Cond"),      # this tells it to transform all columns *except* these ones
                             names_to='Measurement',          # Create a new column called measurements with the original column names
                             values_to='Value')               # Creates a new column with the values from the original rows
```
...
rating_hyp_long_d = rating_hyp_d %>%
  ## your code here
  pivot_longer(cols=-c("Subject", "Cond"),
               names_to='Measurement',        # Call the "Key" column "Measurement"
               values_to='Rating')            # call the "Value" column "Rating"
...
rating_hyp_long_d = rating_hyp_long_d %>%
  mutate(
    ## Your code here
    IsRecall = grepl("Friends|Strangers", Measurement), # create a new variable that will say whether the measurement contain either "Friends" or "Strangers" .,
  )
```
...
rating_summary_d = rating_hyp_long_d %>% 
  ## your code here
  group_by(Emotion, ConfrontationalGame) %>%             # Group emotion and the game type
  summarise(MeanRating = mean(Rating, na.rm=T)) # the na.rm tells R to ignore NA values
```
...
```{r echo=TRUE}
# Took out the NA values by removing it from the z_score_performance column. 
performance_hyp_long_d = performance_hyp_long_d %>%
  filter(!is.na(z_scored_performance))
``` 

```{r echo=TRUE}
# Gives an error. Tried trouubleshooting. 

performance_diff_d = performance_hyp_long_d %>%

  # first, tweak the variable so our code is easier to read.
  mutate(WithMusic = factor(WithMusic, levels=c(F, T), labels=c("PreMusic", "PostMusic"))) %>%

  # now we remove columns we don't need (bonus: leave them in and see if you can understand what goes wrong!)
  select(-c("Score", "Measurement")) %>%
  
  pivot_wider(names_from = WithMusic, values_from = z_scored_performance) %>%
...
performance_diff_summary_d = performance_diff_d %>%
  ## Your code here: Pseudo code since the code chunk above is giving me an error when I tried mutate a new ImprovementScore column 
  group_by(MusicCondition, ConfrontationalGame) %>%                # to get the mean differences within each game and each kind of music
  mutate(MeanImprovementScore = mean(ImprovementScore, na.rm=T))    # the na.rm tells R to ignore NA values
  ungroup()
```
...
```{r echo=TRUE}
# performance_diff_summary_d not found because of the error in the chunk under performance hypthesis (continued)
